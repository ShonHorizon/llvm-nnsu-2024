...
class MGKCilDivConvertPass
    : public PassWrapper<MGKCilDivConvertPass,
                         OperationPass<LLVM::LLVMFuncOp>> {
public:
  StringRef getArgument() const final { return "mgk-cildiv-conv"; }
...
MLIR_DECLARE_EXPLICIT_TYPE_ID(MGKCilDivConvertPass)
MLIR_DEFINE_EXPLICIT_TYPE_ID(MGKCilDivConvertPass)

PassPluginLibraryInfo getMGKCilDivConvertPassPluginInfo() {
  return {MLIR_PLUGIN_API_VERSION, "mgk-cildiv-conv", LLVM_VERSION_STRING,
          []() { PassRegistration<MGKCilDivConvertPass>(); }};
}

extern "C" LLVM_ATTRIBUTE_WEAK PassPluginLibraryInfo mlirGetPassPluginInfo() {
  return getMGKCilDivConvertPassPluginInfo();
}